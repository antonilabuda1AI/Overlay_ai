...
- Instrumentation logs per-tick CSV metrics to `~/.studyglance/logs/perf-<date>.csv`.
...
- OCR_POOL_SIZE: threads for ROI OCR (1–3 recommended)
...
- Small changes (1–3 ROIs): median < 350ms; OCR/ROI < 180ms
- Dropped frames < 10% under bursts with defaults
- DB flush p95 < 10ms (batched)
- Memory stable (< 5% over 10k ticks)

Hotspots
- `ocr/delta.extract_rois`: contour + merge — memory friendly via OpenCV; keep kernel cached
